...
def display_sidebar():
    ...
    with st.sidebar:
        # 利用目的のタイトル
        st.markdown(f"### {ct.SIDEBAR_PURPOSE_TITLE}")
        ...
        st.info(ct.SIDEBAR_MODE1_DESCRIPTION)
        st.code(ct.SIDEBAR_MODE1_EXAMPLE, language=None)
        st.markdown(f"**【「{ct.ANSWER_MODE_2}」を選択した場合】**")
        st.info(ct.SIDEBAR_MODE2_DESCRIPTION)
        st.code(ct.SIDEBAR_MODE2_EXAMPLE, language=None)
